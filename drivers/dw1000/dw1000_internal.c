...
typedef struct
{
    ...
    spi_clk_t SPI_CLK;
    gpio_t pin_irq;
    void (*dwt_txcallback)(netdev_t *dev,const dwt_callback_data_t *txd);
    void (*dwt_rxcallback)(netdev_t *dev,const dwt_callback_data_t *rxd);
...
void set_SPI_pins(dw1000_t *dev){
    //todo read from dev params
    dw1000local.SPI_BUS = (SPI_DEV(1));
    dw1000local.SPI_CS  = (GPIO_PIN(PORT_A, 8));
    gpio_init(dw1000local.SPI_CS, GPIO_OUT);

    dw1000local.SPI_MODE = (SPI_MODE_0);
    dw1000local.SPI_CLK = SPI_CLK_1MHZ;
    dw1000local.pin_irq =  (GPIO_PIN(PORT_B, 11));
    //DEBUG("SPI device %d : cs pin %d  \r\n",(int)dw1000local.SPI_BUS,(int)dw1000local.SPI_CS);
}
/*! ------------------------------------------------------------------------------------------------------------------
 * @fn dwt_initialise()
 *
...
//ported
int dwt_initialise(dw1000_t *dev,uint16 config)
{
    int i = 0;
...
void port_SPIx_set_chip_select(void){
    gpio_init(dw1000local.SPI_CS, GPIO_OUT);
    gpio_write(dw1000local.SPI_CS,1);
}
void port_SPIx_clear_chip_select(void){
    gpio_init(dw1000local.SPI_CS, GPIO_OUT);
    gpio_write(dw1000local.SPI_CS,0);
}
/*! ------------------------------------------------------------------------------------------------------------------
...
uint32 dwt_read32bitoffsetreg(int regFileID,int regOffset)
{
    ...
    {
        //DEBUG("read32bit : read successfully %x:%x:%x:%x\r\n",buffer[0],buffer[1],buffer[2],buffer[3]);
        for (j = 3 ; j >= 0 ; j --)
        {
...
void dwt_enableautoack(uint8 responseDelayTime)
{
    ...
    dwt_enableframefilter(DWT_FF_COORD_EN); //TODO: there is no good documentaiton about framefiltering
    dwt_write32bitreg(SYS_CFG_ID,dw1000local.sysCFGreg) ;
}
void dwt_disableautoack(void){
    dw1000local.sysCFGreg &= ~(SYS_CFG_AUTOACK);
    dwt_enableframefilter(DWT_FF_NOTYPE_EN); //TODO: there is no good documentaiton about framefiltering
    dwt_write32bitreg(SYS_CFG_ID,dw1000local.sysCFGreg) ;
}
 ...
 * no return value
 */
void dwt_setcallbacks(void (*txcallback)(netdev_t *dev,const dwt_callback_data_t *), void (*rxcallback)(netdev_t *dev,const dwt_callback_data_t *))
{
...
void dwt_isr(netdev_t *dev) // assume interrupt can supply context
{
    //puts("dwt isr \r\n");
    uint32  status = 0;
        ...
        if((status & (SYS_STATUS_LDEDONE | SYS_STATUS_RXPHD | SYS_STATUS_RXSFDD)) != (SYS_STATUS_LDEDONE | SYS_STATUS_RXPHD | SYS_STATUS_RXSFDD))
        {
            ...
            else
            {
                ...
                if(dw1000local.dwt_rxcallback != NULL)
                    dw1000local.dwt_rxcallback(dev,&dw1000local.cdata);
            }
        }
    }
    ...
    {
        //puts("RX frame received or RX timeout and if so call the rx callback function\r\n");
...
//                if(len > 0)
//                    puts("dwt isr : frame received\r\n");
//                else
//                    puts("rx isr but rec frame lenght is 0 \r\n");
                ...
                {
                    //puts("dwt isr : double buffer is zero so clear rx events \r\n");
    ...
    if (status & SYS_STATUS_TXFRS)  // Transmit Frame Sent
    {
        ...
        {
            dw1000local.dwt_txcallback(dev,&dw1000local.cdata);
        }

    }
    else if (status & SYS_STATUS_RXRFTO)                                              // Receiver Frame Wait timeout:
    {
        puts("receiver frame wait time out \r\n");
    ...
    else if(status & CLEAR_ALLRXERROR_EVENTS)//catches all other error events
    {
        ...
        if(!(dw1000local.sysCFGreg & SYS_CFG_RXAUTR))
        {
            DEBUG("LDE done flag gets latched on a bad frame\r\n");
            dwt_forcetrxoff(); //this will clear all events
        }
        DEBUG("reset the rx \r\n");
        dwt_rxreset();	//reset the RX
...
void dwt_forcetrxoff(void)
{
    ...
    //DEBUG("sys mask value %x \r\n",(unsigned)mask);
    // need to beware of interrupts occurring in the middle of following read modify write cycle
    // we can disable the radio, but before the status is cleared an interrupt can be set (e.g. the
    // event has just happened before the radio was disabled)
    // thus we need to disable interrupt during this operation
    stat = decamutexon() ; //todo this should be solved soon hessam
    //DEBUG("writing sys mask id with 0 \r\n");
    dwt_write32bitreg(SYS_MASK_ID, 0) ; //clear interrupt mask - so we don't get any unwanted events
    //DEBUG("truning off the radio \r\n");
    dwt_writetodevice(SYS_CTRL_ID,0,1,&temp) ; //disable the radio

    // Forcing Transceiver off - so we do not want to see any new events that may have happened
    //DEBUG("forcing tranciver off \r\n");
    dwt_write32bitreg(SYS_STATUS_ID,(CLEAR_ALLTX_EVENTS | CLEAR_ALLRXERROR_EVENTS | CLEAR_ALLRXGOOD_EVENTS)) ;
    //DEBUG("syn rx buffers \r\n");
    dwt_syncrxbufptrs();
    //DEBUG("set interupt mask to pervious value \r\n");
    dwt_write32bitreg(SYS_MASK_ID, mask) ; //set interrupt mask to what it was

    //enable/restore interrupts again...
    decamutexoff(stat) ; //todo this should be solved
    dw1000local.wait4resp = 0;
    DEBUG("force trx off is done \r\n");
} // end deviceforcetrxoff()
int decamutexon(void){
    gpio_irq_disable( (GPIO_PIN(PORT_B, 11)));
    return 0;
}
int decamutexoff(int a){
    gpio_irq_enable( (GPIO_PIN(PORT_B, 11)));
    return 0;
}
...
{
   // puts("read from spi \r\n");
    //DEBUG("readfromspi : headerlenght %d : readlength %d : %x \r\n",headerLength,(int)readlength,*headerBuffer);
    spi_acquire(dw1000local.SPI_BUS, dw1000local.SPI_CS, dw1000local.SPI_MODE, dw1000local.SPI_CLK);
    spi_transfer_bytes(dw1000local.SPI_BUS, dw1000local.SPI_CS, true, headerBuffer, NULL, headerLength);
    spi_transfer_bytes(dw1000local.SPI_BUS, dw1000local.SPI_CS, false, NULL, readBuffer, readlength);
    spi_release(dw1000local.SPI_BUS);
    //puts("return from spi \r\n");
    return DWT_SUCCESS;
} // end readfromspi()
...
int writetospi(uint16 headerLength, const uint8 *headerBuffer,uint32 bodyLength, const uint8 *bodyBuffer)
{
    ...
    spi_acquire(dw1000local.SPI_BUS, dw1000local.SPI_CS, dw1000local.SPI_MODE, dw1000local.SPI_CLK);
    spi_transfer_bytes(dw1000local.SPI_BUS, dw1000local.SPI_CS, true, headerBuffer, NULL, headerLength);
    spi_transfer_bytes(dw1000local.SPI_BUS, dw1000local.SPI_CS, false, bodyBuffer, NULL, bodyLength);
    spi_release(dw1000local.SPI_BUS);
    return DWT_SUCCESS;
} // end writetospi()
int rx_read_len(void){
    return dw1000local.cdata.datalength;
}

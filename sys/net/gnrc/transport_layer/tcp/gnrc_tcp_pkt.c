              ...
              const bool retransmit)
{
    ...
    DEBUG("sending packet to network stack with pid %d \r\n",gnrc_tcp_pid);
...
int _pkt_setup_retransmit(gnrc_tcp_tcb_t *tcb, gnrc_pktsnip_t *pkt, const bool retransmit)
{
    ...
    /* RTO adjustment */
    if (!retransmit) {
        DEBUG("first retransmission set up \r\n");
        ...
        DEBUG("not first retransmission set up \r\n");
...
{
    DEBUG("gncr tcp pkt: acknowledge the packet \r\n");
    uint32_t seg = 0;
    gnrc_pktsnip_t *snp = NULL;
    tcp_hdr_t *hdr;

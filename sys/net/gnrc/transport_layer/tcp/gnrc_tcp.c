                      ...
                      const uint32_t timeout_duration_us)
{
    ...
    while (ret == 0) {
        ...
        DEBUG("gnrc tcp receive : calling fsm call rec \r\n");
        /* Try to read available data */
        ret = _fsm(tcb, FSM_EVENT_CALL_RECV, NULL, data, max_len);
        DEBUG("gnrc tcp receive : fsm call rec done with return value %d \r\n",ret);
        /* If there was no data: Wait for next packet or until the timeout fires */
        if (ret <= 0) {
            DEBUG("gnrc tcp receive : before mbox get \r\n");
            mbox_get(&(tcb->mbox), &msg);
            DEBUG("gnrc tcp receive : after mbox get \r\n");
            switch (msg.type) {
                case MSG_TYPE_CONNECTION_TIMEOUT:

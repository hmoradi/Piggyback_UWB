                                ...
                                gnrc_nettype_t type)
{
    ...
    //DEBUG("gnrc pktbuf add : locking mutex \r\n");
    mutex_lock(&_mutex);
   // DEBUG("gnrc pktbuf add : locked mutex \r\n");
    pkt = _create_snip(next, data, size, type);
    //DEBUG("gnrc pktbuf add : unlocking mutex \r\n");
    mutex_unlock(&_mutex);
    //DEBUG("gnrc pktbuf add : unlocked mutex \r\n");
    return pkt;
}
...
static gnrc_pktsnip_t *_create_snip(gnrc_pktsnip_t *next, void *data, size_t size,
                                    gnrc_nettype_t type)
{
    //DEBUG("creating sniptet \r\n");
    gnrc_pktsnip_t *pkt = _pktbuf_alloc(sizeof(gnrc_pktsnip_t));
    //DEBUG("creating sniptet allocating done \r\n");
    void *_data = NULL;
    ...
    if (size > 0) {
        ...
        //DEBUG("creating sniptet second allocation done \r\n");
        if (_data == NULL) {
    ...
    //DEBUG("creating sniptet setting pktsnip\r\n");
    _set_pktsnip(pkt, next, _data, size, type);
    if (data != NULL) {
        memcpy(_data, data, size);
    }
    //DEBUG("creating sniptet done \r\n");
    return pkt;
}

...
{
    //DEBUG("ieeee 802 init gnrc \r\n");
    gnrc_netdev->send = _send;
    gnrc_netdev->recv = _recv;
    //DEBUG("initi gnrc recv to  %x )))))))) \r\n",(uintptr_t)(*(gnrc_netdev->recv)));
    //DEBUG("gnrc pid is %d \r\n",gnrc_netdev->pid);
    gnrc_netdev->dev = (netdev_t *)dev;
    //DEBUG("ieeee 802 init gnrc done &&&&&&&&&&&&&&&&&&&&&&\r\n");
    return 0;
}
...
static gnrc_pktsnip_t *_recv(gnrc_netdev_t *gnrc_netdev)
{
    DEBUG("ieee802 rec is called \r\n");
    netdev_t *netdev = (netdev_t *)(gnrc_netdev->dev);
    netdev_ieee802154_rx_info_t rx_info;
    netdev_ieee802154_t *state = (netdev_ieee802154_t *)gnrc_netdev->dev;
    gnrc_pktsnip_t *pkt = NULL;
    DEBUG("ieee802 rec before calling the driver rec \r\n");
    int bytes_expected = netdev->driver->recv(netdev, NULL, 0, NULL);
    DEBUG("ieee 802 rec bytes epxeted %d \r\n",bytes_expected);
    if (bytes_expected > 0) {
        int nread;
        DEBUG("ieee 802 rec create the pkt \r\n");
        pkt = gnrc_pktbuf_add(NULL, NULL, bytes_expected, GNRC_NETTYPE_UNDEF);
        if (pkt == NULL) {
            DEBUG("_recv_ieee802154: cannot allocate pktsnip.\n");
            return NULL;
        }
        DEBUG("ieee 802 reading the packet \r\n");
        nread = netdev->driver->recv(netdev, pkt->data, bytes_expected, &rx_info);
        if (nread <= 0) {
            gnrc_pktbuf_release(pkt);
            DEBUG("ieee 802 error reading the packet  \r\n");
            return NULL;
        }
...
static int _send(gnrc_netdev_t *gnrc_netdev, gnrc_pktsnip_t *pkt)
{
    ...
    if (netif_hdr->flags & /* If any of these flags is set so this is correct */
        ...
        DEBUG("ieee802154->send -> broadcast \r\n");
    }
    else {
    ...
    if (src_len > 0) {
       ...
       // DEBUG("ieee8025: src addr is  %c \r\n",*src);
    }
    else if (state->flags & NETDEV_IEEE802154_SRC_MODE_LONG) {
        //DEBUG("ieee 802 bad luck src mod long \r\n");
        ...
        DEBUG("ieed 802 ; set to short address %c\r\n",*src);
    }

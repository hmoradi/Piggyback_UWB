...
{
    DEBUG("Setting up the net driver \r\n");
    ...
    if(*s==1) {
        ...
        netdev->driver->set(netdev, NETOPT_RX_END_IRQ, 0, 0);
    }
    dev->state = state;
    mutex_init(&dev->mutex);
    netdev_test_reset(dev);
    DEBUG("Setting up the net driver is done \r\n");
}

void netdev_test_reset(netdev_test_t *dev)
{
    DEBUG("reseting net dev  \r\n");
    ...
    DEBUG("resetting net dev is done \r\n");
}
